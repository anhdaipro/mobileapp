...
const Updatevariation=(props)=>{
    const {route,navigation}=props
    const {item_id,headers}=route.params
    const [variations,setVariations]=useState([])
    useEffect(() => {
        (async()=>{
            try{
            const res= await axios.get(`${updatevariationURL}?item_id=${item_id}`,headers())
    ...
    const colors=useMemo(()=>{
        return variations.reduce((arr,obj)=>{
            const color=obj['color_value']
    ...
    const submit= async ()=>{
        const res =await axios.post(updatevariationURL,JSON.stringify(variations),headers())
        navigation.goBack()
    }
    return(
        <Container style={{backgroundColor:'transparent'}}>
            ...
            <View>
                ...
                <View style={{paddingBottom:100}}>
                    <ScrollView>
                        {colors.map(color=>
                        <View key={color} style={{paddingBottom:8,paddingTop:8,marginBottom:8}}>
                            <View style={{paddingBottom:8,borderColor:'rgba(0,0,0,0.1)',borderBottomWidth:1}}>
                               <Text>{color}</Text>
                            </View>
                            <View>
                            {variations.filter(variation=>variation.color_value==color).map(variation=>
                                <View style={[styles.flexcenter,{padding:8,borderColor:'rgba(0,0,0,0.1)',borderBottomWidth:1}]}>
                                    <View style={{flex:2}}>
