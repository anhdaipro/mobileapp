...
import {listAddonshopURL,} from "../../../../urls"
    ...
    const Gift=({item})=>{
        return(
            <View style={{padding:8}}>
                ...
                <View>
                    <View><Text>Mua để nhận quà (4)</Text></View>
                    <View style={styles.item_space,{padding:8}}>
                        <View style={{position:'relative'}}>
                            {item.main_products.map((product,index)=>{
                                if(index<2){
                                return(
                            ...
                            {item.main_products.length>2?
                            <Itemlast>
                                <Text style={{color:'#fff',textAlign:'center',fontSize:12}}>+{item.main_products.length-4}</Text>
                            </Itemlast>:null}
                        </View>
                        <View>
                            <View>
                                <Text>Giá trị đơn tối thiểu: </Text>
                                <Text style={styles.textorange}>{item.minimun_order_value}</Text>
                            </View>
                            <View>
                                <Text>Số lượng quà tặng:</Text>
                                <Text style={styles.textorange}>{item.byproducts.length}</Text>
                            </View>
                        </View>
                    </View>
                </View>
                <View>
                    ...
                    <StyleBtn onPress={()=>navigation.navigate("DetailGift",{id:item.id,headers:headers})} style={{marginTop:8}}>
                        <Text>Xem</Text>
                    </StyleBtn>:
                    <>
                        <StyleBtn onPress={()=>navigation.navigate("DetailGift",{id:item.id,headers:headers})} style={{marginTop:8}}>
    ...
    return(
        <Container>
            ...
            <View >
                <View >   
                    ...
                    <Wrapper>
                        <FlatList
                            data={listdeal}
                            renderItem={({item})=><Gift item={item}/>}
            ...
            <View style={[styles.fotter,{padding:8}]}>             
                 <StyleBtn onPress={()=>navigation.navigate("DetailGift",{headers:headers})} style={{flex:1}}>
                     <Text>Tạo mới</Text>
                </StyleBtn>  
            </View> 
        </Container>
