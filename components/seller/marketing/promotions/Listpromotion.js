            ...
            <View style={{padding:8}}>
                <Flexrow>
                    <View style={{marginRight:8}}>
                        ...
                        <View>
                            <Text data-v-771d39f6="">Mua 
                                <Text> {item.quantity_to_reduced}</Text> sản phẩm {item.combo_type=='3'?'chỉ với giá':'để được giảm'}
                                <Text>{item.combo_type=='1'?` ${item.discount_percent}%GIẢM`:` ₫${formatter.format(item.combo_type=='2'?item.discount_price:item.price_special_sale)}`}</Text>
                            </Text>
                        </View>
                        <View>
                ...
                <View>
                    ...
                    <StyleBtn onPress={()=>navigation.navigate("Detailcombo",{id:item.id,headers:headers})} style={{marginTop:8}}>
                        <Text>Xem</Text>
                    </StyleBtn>:
                    <>
                        <StyleBtn onPress={()=>navigation.navigate("Detailcombo",{id:item.id,headers:headers})} style={{marginTop:8}}>
    ...
    return(
        <Container>
            ...
            <View >
                <View >   
                    ...
                    <Wrapper>
                        <FlatList
                            data={listcombo}
                            renderItem={({item})=><Combo item={item}/>}
            ...
            <View style={[styles.fotter,{padding:8}]}>             
                 <StyleBtn onPress={()=>navigation.navigate("Detailcombo",{headers:headers})} style={{flex:1}}>
                     <Text>Tạo mới</Text>
                </StyleBtn>  
            </View> 
        </Container>
